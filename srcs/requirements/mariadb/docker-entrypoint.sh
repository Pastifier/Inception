#!/bin/sh

set -e

if [ ! -f "/var/lib/mysql/ib_buffer_pool" ];
then
	/etc/init.d/mariadb setup
	rc-service mariadb start

    if rc-service mariadb status; then
        echo "service: mariadb: already running."
    else
        rc-service mariadb start
    fi


    # Create local user and database for wordpress
	mysql -u $MYSQL_ROOT_USER -e "CREATE USER '$DB_USER'@'localhost' IDENTIFIED BY '$DB_PASSWORD';"
	mysql -u $MYSQL_ROOT_USER -e "GRANT ALL PRIVILEGES ON *.* TO '$DB_USER'@'localhost' WITH GRANT OPTION;"
	mysql -u $MYSQL_ROOT_USER -e "FLUSH PRIVILEGES;"
    mysql -u $MYSQL_ROOT_USER -e "CREATE DATABASE $DB_NAME DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci;"

	# Create new remote user and Make The user GRANT ALL PRIVILEGES on Wordpress database
	mysql -u $MYSQL_ROOT_USER -e "CREATE USER '$DB_USER'@'%' IDENTIFIED BY '$DB_PASSWORD';"
	mysql -u $MYSQL_ROOT_USER -e "GRANT ALL PRIVILEGES ON $DB_NAME.* TO '$DB_USER'@'%' IDENTIFIED BY '$DB_PASSWORD';"
	mysql -u $MYSQL_ROOT_USER -e "FLUSH PRIVILEGES;"

	mysql -u $MYSQL_ROOT_USER -e "ALTER USER '$MYSQL_ROOT_USER'@'localhost' IDENTIFIED BY '$MYSQL_ROOT_PASS';"

fi


# # Comment skip-networking
sed -i 's/skip-networking/# skip-networking/g' /etc/my.cnf.d/mariadb-server.cnf
sed -i 's/#bind-address=0.0.0.0/bind-address=0.0.0.0/g' /etc/my.cnf.d/mariadb-server.cnf

rc-service mariadb restart
rc-service mariadb stop

exec /usr/bin/mariadbd --basedir=/usr --datadir=/var/lib/mysql --plugin-dir=/usr/lib/mariadb/plugin --user=mysql --pid-file=/run/mysqld/mariadb.pid